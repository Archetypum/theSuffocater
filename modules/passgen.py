...
def passgen() -> None:
    ...
    if answer in ["y", "yes"]:
        ...
        with open("config_files/passgen_dict.txt", "r") as words_dict:
            word_list = [word.strip().strip("'") for word in words_dict.read().split(",")]

        created_password = "".join(secrets.choice(characters) for _ in range(password_length)) + secrets.choice(word_list)
